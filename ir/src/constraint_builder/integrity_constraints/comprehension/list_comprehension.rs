use super::{
    build_iterable_context, ComprehensionType, ConstraintBuilder, Expression, ListComprehension,
    SemanticError,
};
...
impl ConstraintBuilder {
    ...
    pub fn unfold_lc(&self, lc: &ListComprehension) -> Result<Vec<Expression>, SemanticError> {
        let num_iterations = self.get_num_iterations(&ComprehensionType::List, lc.context())?;
        if num_iterations == 0 {
        ...
        let iterable_context =
            build_iterable_context(&ComprehensionType::List, &lc.context().to_vec())?;
