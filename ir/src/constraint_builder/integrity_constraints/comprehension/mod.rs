...
impl Display for ComprehensionType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ComprehensionType::List => write!(f, "list"),
            ComprehensionType::Constraint => write!(f, "constraint"),
        }
    }
}
...
impl ConstraintBuilder {
    ...
    fn parse_comprehension_expr(
        ...
        match expression {
            Expression::SymbolAccess(symbol_access) => {
                self.parse_symbol_access(comprehension_type, symbol_access, iterable_context, i)
            }
            Expression::Add(lhs, rhs) => {
                let lhs =
                    self.parse_comprehension_expr(comprehension_type, lhs, iterable_context, i)?;
                let rhs =
                    self.parse_comprehension_expr(comprehension_type, rhs, iterable_context, i)?;
            ...
            Expression::ListFolding(lf_type) => {
                self.parse_list_folding(comprehension_type, lf_type, expression, i)
            }
        ...
        match iterable {
            ...
            Some(iterable_type) => match iterable_type {
                Iterable::Identifier(ident) => {
                    ...
                    build_ident_expression(comprehension_type, symbol, i, symbol_access.offset())
                }
            ...
            ListFolding::Sum(lf_value_type) | ListFolding::Prod(lf_value_type) => {
                ...
                let iterable_context =
                    if let ListFoldingValueExpr::ListComprehension(lc) = lf_value_type {
                        build_iterable_context(comprehension_type, &lc.context().to_vec())?
                    } else {
                        BTreeMap::new()
                    };
                if list.is_empty() {
                ...
                let mut acc = self.parse_comprehension_expr(
                    comprehension_type,
                    expression,
                    &iterable_context,
                    i,
    ...
    fn get_num_iterations(
        ...
        let comprehension_len =
            self.get_iterable_len(comprehension_type, &comprehension_context[0].1)?;
        for (_, iterable) in comprehension_context.iter().skip(1) {
            let iterable_len = self.get_iterable_len(comprehension_type, iterable)?;
...
fn validate_access(
    ...
    if i < size {
        ...
        Err(SemanticError::IndexOutOfRange(format!(
            "Invalid access index {i} used in {comprehension_type} comprehension"
        )))
    }
}
    ...
    for (member, iterable) in comprehension_context {
            ...
            .is_some()
        {
            return Err(SemanticError::InvalidComprehension(format!(
                "Duplicate member {member} in {comprehension_type} comprehension"
            )));
        }
    }
    Ok(iterable_context)
...
fn build_ident_expression(
    ...
    match symbol.binding() {
        SymbolBinding::Trace(trace_columns) => {
            validate_access(comprehension_type, i, trace_columns.size())?;
        ...
        SymbolBinding::Variable(variable_type) => {
            match variable_type {
                VariableValueExpr::Vector(vector) => {
                    validate_access(comprehension_type, i, vector.len())?;
                    Ok(vector[i].clone())
                }
        ...
        SymbolBinding::PublicInput(size) | SymbolBinding::RandomValues(_, size) => {
            validate_access(comprehension_type, i, *size)?;
