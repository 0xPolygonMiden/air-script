...
// TODO: test trait for building inputs and building expected?
// takes generated data and returns the inputs and the expected result
#[cfg(test)]
mod tests {
    use super::{
        Identifier, IdentifierType, PeriodicColumn, PublicInput, SymbolTable, TraceColumn,
    };
    ...
    fn test_main() {
        ...
        let columns = main.map(|name| Identifier(name.to_string()));
        let res = sym_table.insert_trace_columns(0, &columns);

        // trace columns were added successfully.
        assert!(res.is_ok());

        // the symbol table has the expected identifier information.
        for (idx, &col_name) in main.iter().enumerate() {
            assert_eq!(
                sym_table.identifiers.get(col_name),
                Some(&IdentifierType::TraceColumn(TraceColumn::new(0, idx)))
            );
        }
    }
    ...
    fn error_main_dup() {
        ...
        // insert duplicate main trace columns
        let dup = "dup".to_string();
        let columns = [Identifier(dup.clone()), Identifier(dup)];
        sym_table.insert_trace_columns(0, &columns).unwrap();
    ...
    fn test_aux() {
        ...
        let mut sym_table = SymbolTable::default();
        let columns = aux.map(|name| Identifier(name.to_string()));
        let res = sym_table.insert_trace_columns(1, &columns);

        // trace columns were added successfully.
        assert!(res.is_ok());

        // the symbol table has the expected identifier information.
        for (idx, &col_name) in aux.iter().enumerate() {
            assert_eq!(
                sym_table.identifiers.get(col_name),
                Some(&IdentifierType::TraceColumn(TraceColumn::new(1, idx)))
            );
        }
    }
    ...
    fn test_pub_inputs() {
        // the columns to test.
        let names = ["a", "b", "third"];
        let sizes = [4_u64, 8, 16];

        // insert public inputs.
        let mut sym_table = SymbolTable::default();

        let pub_inputs = names
            .iter()
            .zip(sizes.iter())
            .map(|(&name, &size)| PublicInput::new(Identifier(name.to_string()), size))
            .collect::<Vec<_>>();

        let res = sym_table.insert_public_inputs(&pub_inputs);

        // public inputs were added successfully.
        assert!(res.is_ok());

        // the symbol table has the expected identifier information.
        for (idx, &col_name) in names.iter().enumerate() {
            assert_eq!(
                sym_table.identifiers.get(col_name),
                Some(&IdentifierType::PublicInput(sizes[idx] as usize))
            );
        }

        // the symbol table has the expected public inputs.
        let expected_pub_inputs = names
            .iter()
            .zip(sizes.iter())
            .map(|(&name, &size)| (name.to_string(), size as usize))
            .collect::<Vec<_>>();
    ...
    fn test_periodic_columns() {
        // the columns to test.
        let names = ["a", "b", "third"];
        let values = vec![vec![1_u64, 0], vec![1, 0, 0, 0], vec![1, 1, 1, 0]];

        // insert public inputs.
        let mut sym_table = SymbolTable::default();

        let periodic_cols = names
            .iter()
            .zip(values.iter())
            .map(|(name, values)| {
        ...
        let res = sym_table.insert_periodic_columns(&periodic_cols);
        ...
        for (idx, &col_name) in names.iter().enumerate() {
            assert_eq!(
                sym_table.identifiers.get(col_name),
                Some(&IdentifierType::PeriodicColumn(idx, values[idx].len()))
            );
        }

        // the symbol table has the expected periodic columns.
