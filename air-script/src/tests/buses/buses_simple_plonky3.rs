      ...
      EF: ExtensionField<F>,
{
    ...
    where AB: MidenAirBuilder<F = F>,
    {
        ...
        builder.when_transition().assert_zero_ext((alpha.into() + beta_challenges[0].into() + beta_challenges[1].into().double()) * AB::ExprEF::from(aux_next[1].clone().into()) + AB::ExprEF::from_u64(2) - ((alpha.into() + beta_challenges[0].into() + beta_challenges[1].into().double()) * AB::ExprEF::from(aux_current[1].clone().into()) + AB::ExprEF::from(main_current[0].clone().into())));
    }
}
...
impl BusesAir {
          ...
          EF: ExtensionField<F>,
    {
        ...
        let _periodic_values: [_; NUM_PERIODIC_VALUES] = periodic_evals.try_into().expect("Wrong number of periodic values");
        (challenges[0] + challenges[1] * EF::ONE + challenges[2] * EF::from_u64(2)) * (challenges[0] + challenges[1] * EF::ONE + challenges[2] * EF::from_u64(2))
    }
          ...
          EF: ExtensionField<F>,
    {
        ...
        let _periodic_values: [_; NUM_PERIODIC_VALUES] = periodic_evals.try_into().expect("Wrong number of periodic values");
        let terms_added = EF::from(main_current[0].clone()) * (challenges[0] + challenges[1] * EF::ONE + challenges[2] * EF::from_u64(2));
        let terms_removed = EF::from_u64(2) * (challenges[0] + challenges[1] * EF::ONE + challenges[2] * EF::from_u64(2));
