...
pub const MAX_BETA_CHALLENGE_POWER: usize = 3;
      ...
      EF: ExtensionField<F>,
{
    ...
    where AB: MidenAirBuilder<F = F>,
    {
        ...
        builder.when_transition().assert_zero_ext((alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[0].clone().into()) * beta_challenges[1].into()) * (alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[1].clone().into()) * beta_challenges[1].into()) * AB::ExprEF::from(aux_next[1].clone().into()) + AB::ExprEF::from(main_current[6].clone().into()) * (alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[0].clone().into()) * beta_challenges[1].into()) - ((alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[0].clone().into()) * beta_challenges[1].into()) * (alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[1].clone().into()) * beta_challenges[1].into()) * AB::ExprEF::from(aux_current[1].clone().into()) + (AB::ExprEF::from(main_current[4].clone().into()) + AB::ExprEF::from(main_current[5].clone().into())) * (alpha.into() + AB::ExprEF::from_u64(3) * beta_challenges[0].into() + AB::ExprEF::from(main_current[1].clone().into()) * beta_challenges[1].into())));
    }
}
...
impl BusesAir {
          ...
          EF: ExtensionField<F>,
    {
        ...
        let result = ((challenges[0] + challenges[1] * EF::ONE + challenges[2] * (EF::from_u64(3) + EF::from(main_current[1].clone())) + challenges[3] * EF::from(main_current[1].clone())) * EF::from(main_current[3].clone()) + (EF::ONE - EF::from(main_current[3].clone()))) * ((challenges[0] + challenges[1] * EF::from_u64(2) + challenges[2] * EF::from(main_current[0].clone())) * (EF::ONE - EF::from(main_current[3].clone())) + (EF::ONE - (EF::ONE - EF::from(main_current[3].clone()))));
        if result == EF::ZERO {
            return EF::ONE;
        }
        result
          ...
          EF: ExtensionField<F>,
    {
        ...
        let _periodic_values: [_; NUM_PERIODIC_VALUES] = periodic_evals.try_into().expect("Wrong number of periodic values");
        let result = ((challenges[0] + challenges[1] * EF::ONE + challenges[2] * (EF::from_u64(3) + EF::from(main_current[1].clone())) + challenges[3] * EF::from(main_current[0].clone())) * EF::from(main_current[2].clone()) + (EF::ONE - EF::from(main_current[2].clone()))) * ((challenges[0] + challenges[1] * EF::from_u64(2) + challenges[2] * EF::from(main_current[1].clone())) * (EF::ONE - EF::from(main_current[2].clone())) + (EF::ONE - (EF::ONE - EF::from(main_current[2].clone()))));
        if result == EF::ZERO {
            return EF::ONE;
        }
        result
          ...
          EF: ExtensionField<F>,
    {
        ...
        let _periodic_values: [_; NUM_PERIODIC_VALUES] = periodic_evals.try_into().expect("Wrong number of periodic values");
        (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone())) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone())) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[1].clone()))
    }
          ...
          EF: ExtensionField<F>,
    {
        ...
        let _periodic_values: [_; NUM_PERIODIC_VALUES] = periodic_evals.try_into().expect("Wrong number of periodic values");
        let terms_added = EF::from(main_current[4].clone()) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone())) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[1].clone())) + EF::from(main_current[5].clone()) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone())) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[1].clone()));
        let terms_removed = EF::from(main_current[6].clone()) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone())) * (challenges[0] + challenges[1] * EF::from_u64(3) + challenges[2] * EF::from(main_current[0].clone()));
