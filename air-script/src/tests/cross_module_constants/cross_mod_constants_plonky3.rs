...
impl<F, EF> MidenAir<F, EF> for CrossModuleConstantsTest {
    fn width(&self) -> usize {
        MAIN_WIDTH
    }
    ...
    where AB: MidenAirBuilder<F = F>,
    {
        ...
        builder.assert_zero(main_current[4].clone().into() - (main_current[0].clone().into() + main_current[1].clone().into().double() + main_current[2].clone().into() * AB::Expr::from_u64(3) + main_current[3].clone().into() * AB::Expr::from_u64(4)));
