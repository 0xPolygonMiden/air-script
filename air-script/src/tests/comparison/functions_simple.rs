//! Cross-backend comparison test for the FunctionsSimple AIR.
//!
//! This test verifies that Winterfell and Plonky3 produce equivalent
//! constraint evaluations for the FunctionsSimple AIR at every row of the trace.
...
/// Configuration for FunctionsSimple AIR cross-backend comparison tests.
struct FunctionsSimpleTestConfig {
    /// The trace length.
    trace_length: usize,
}

impl FunctionsSimpleTestConfig {
...
impl CrossBackendTestConfig for FunctionsSimpleTestConfig {
...
#[test]
fn test_functions_simple_air_constraint_comparison() {
    let config = FunctionsSimpleTestConfig::new(64);
    ...
    println!(
        "FunctionsSimple AIR comparison passed: {} constraints checked across {} rows",
        result.total_constraints_checked, result.total_rows
    );
}

#[test]
fn test_functions_simple_air_constraint_comparison_random_inputs() {
    let config = FunctionsSimpleTestConfig::new(64);
    ...
    for iteration in 0u64..=50 {
        let result = run_comparison(
            &config,
            TraceSource::Random {
                test_name: "test_functions_simple_air_constraint_comparison_random_inputs",
                iteration,
            },
        );
    ...
    println!("FunctionsSimple AIR random comparison passed for all 51 iterations (0-50)");
}
