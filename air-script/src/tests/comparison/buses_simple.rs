//! Cross-backend comparison test for the BusesSimple AIR.
//!
//! This test verifies that Winterfell and Plonky3 produce equivalent
//! constraint evaluations for the BusesSimple AIR at every row of the trace.
...
/// Configuration for BusesSimple AIR cross-backend comparison tests.
struct BusesSimpleTestConfig {
    /// The trace length.
    trace_length: usize,
}

impl BusesSimpleTestConfig {
...
impl CrossBackendTestConfig for BusesSimpleTestConfig {
...
fn test_buses_simple_air_constraint_comparison() {
    let config = BusesSimpleTestConfig::new(64);
    ...
    println!(
        "BusesSimple AIR comparison passed: {} constraints checked across {} rows",
...
#[test]
#[ignore = "TODO: Winterfell codegen uses empty main degrees/assertions; fix goldens or codegen to satisfy AirContext::new_multi_segment"]
fn test_buses_simple_air_constraint_comparison_random_inputs() {
    let config = BusesSimpleTestConfig::new(64);
    ...
    for iteration in 0u64..=50 {
        let result = run_comparison(
            &config,
            TraceSource::Random {
                test_name: "test_buses_simple_air_constraint_comparison_random_inputs",
                iteration,
            },
        );
    ...
    println!("BusesSimple AIR random comparison passed for all 51 iterations (0-50)");
}
