...
impl<F: Field, AB: AirScriptBuilder<F = F>> AirScriptAir<F, AB> for EvaluatorsAir {
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero(main_next[0].clone().into() - main_current[0].clone().into());
        builder.when_transition().assert_zero(main_next[2].clone().into() - main_current[2].clone().into());
        builder.when_transition().assert_zero(main_next[6].clone().into() - main_current[6].clone().into());
        ...
        builder.assert_zero(main_current[4].clone().into());
