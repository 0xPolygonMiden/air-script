...
use crate::{
    ...
    tests::evaluators::evaluators_plonky3::{EvaluatorsAir, MAIN_WIDTH},
};
...
pub fn generate_trace_rows<F: PrimeField64>(inputs: Vec<u64>) -> RowMajorMatrix<F> {
    ...
    rows[0][4] = F::ZERO;
    rows[0][5] = F::ONE;
    rows[0][6] = F::from_canonical_checked(4).unwrap();
...
generate_air_plonky3_test_with_airscript_traits!(test_air_plonky3, EvaluatorsAir);
