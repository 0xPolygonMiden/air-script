...
use crate::{
    ...
    trace_col_groups::trace_col_groups_plonky3::{NUM_COLUMNS, TraceColGroupAir},
};
...
pub fn generate_trace_rows<F: PrimeField64>(inputs: Vec<u32>) -> RowMajorMatrix<F> {
    ...
    for i in 1..num_rows {
        ...
        rows[i][0] = col_0_prev;
        rows[i][1] = col_1_prev - F::ONE;
        rows[i][2] = col_2_prev + F::ONE;
        rows[i][3] = col_3_prev;
        rows[i][4] = col_4_prev;
        rows[i][5] = col_5_prev;
        rows[i][6] = col_6_prev;
...
fn test_air_plonky3() {
    ...
    check_constraints_with_periodic_columns(&TraceColGroupAir {}, &trace, &inputs_m31);
}
