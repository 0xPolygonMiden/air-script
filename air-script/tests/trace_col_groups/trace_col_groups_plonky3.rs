...
impl<AB: AirBuilderWithPublicValues + AirBuilderWithPeriodicColumns> Air<AB> for TraceColGroupAir {
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero::<_>(main_next[2].into() - (main_current[2].into() + AB::Expr::ONE));
        builder.when_transition().assert_zero::<_>(main_next[1].into() - (main_current[1].into() - AB::Expr::ONE));
    }
}