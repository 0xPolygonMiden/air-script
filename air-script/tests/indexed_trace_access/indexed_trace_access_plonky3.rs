...
impl<F: Field, AB: AirScriptBuilder<F = F>> AirScriptAir<F, AB> for TraceAccessAir {
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero_ext::<_>(main_next[0].clone().into() - (main_current[1].clone().into() + AB::Expr::ONE));
    }
}

impl<AB: AirScriptBuilder> Air<AB> for TraceAccessAir {
    fn eval(&self, builder: &mut AB) {
