...
impl<F: Field> AirScriptAir<F> for FibonacciAir {
    ...
    where AB: AirScriptBuilder<F = F>,
    {
        ...
        builder.when_first_row().assert_zero_ext::<_>(main_current[0].clone().into() - public_values[0].into());
        builder.when_first_row().assert_zero_ext::<_>(main_current[1].clone().into() - public_values[1].into());
        builder.when_last_row().assert_zero_ext::<_>(main_current[1].clone().into() - public_values[2].into());
        builder.when_transition().assert_zero_ext::<_>(main_next[1].clone().into() - (main_current[0].clone().into() + main_current[1].clone().into()));
        builder.when_transition().assert_zero_ext::<_>(main_next[0].clone().into() - main_current[1].clone().into());
    }
}
