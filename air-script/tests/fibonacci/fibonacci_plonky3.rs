...
impl<AB: AirBuilderWithPublicValues + AirBuilderWithPeriodicColumns> Air<AB> for FibonacciAir {
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_first_row().assert_zero::<_>(main_current[1] - public_values[1].into());
        builder.when_last_row().assert_zero::<_>(main_current[1] - public_values[2].into());
        builder.when_transition().assert_zero::<_>(main_next[1] - (main_current[0] + main_current[1]));
        builder.when_transition().assert_zero::<_>(main_next[0] - main_current[1]);
    }
}