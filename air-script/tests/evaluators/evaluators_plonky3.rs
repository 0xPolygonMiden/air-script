...
impl<F: Field, AB: AirScriptBuilder<F = F>> AirScriptAir<F, AB> for EvaluatorsAir {
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero_ext::<_>(main_next[0].clone().into() - main_current[0].clone().into());
        builder.when_transition().assert_zero_ext::<_>(main_next[2].clone().into() - main_current[2].clone().into());
        builder.when_transition().assert_zero_ext::<_>(main_next[6].clone().into() - main_current[6].clone().into());
        builder.assert_zero_ext::<_>(main_current[0].clone().into() * main_current[0].clone().into() - main_current[0].clone().into());
        builder.assert_zero_ext::<_>(main_current[1].clone().into() * main_current[1].clone().into() - main_current[1].clone().into());
        ...
        builder.assert_zero_ext::<_>(main_current[4].clone().into());
        builder.assert_zero_ext::<_>(main_current[5].clone().into() - AB::Expr::ONE);
        builder.assert_zero_ext::<_>(main_current[6].clone().into() - AB::Expr::from_u64(4));
    }
}

impl<AB: AirScriptBuilder> Air<AB> for EvaluatorsAir {
    fn eval(&self, builder: &mut AB) {
