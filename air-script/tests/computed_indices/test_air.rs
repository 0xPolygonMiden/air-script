...
use crate::{
    computed_indices::computed_indices_simple::{ComputedIndicesAir, PublicInputs},
...
#[test]
fn test_computed_indices_air() {
    let air_tester = Box::new(ComputedIndicesAirTester {});
    let length = 1024;
    ...
    let air = ComputedIndicesAir::new(trace_info, pub_inputs, options);
    main_trace.validate::<ComputedIndicesAir, Felt>(&air, aux_trace.as_ref());
}
