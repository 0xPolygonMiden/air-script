...
impl<AB: AirBuilderWithPublicValues + AirBuilderWithPeriodicColumns> Air<AB> for SelectorsAir {
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero::<_>(main_next[1].clone().into() - main_current[2].clone().into());
        builder.when_transition().assert_zero::<_>(main_current[3].clone().into() * (main_next[0].clone().into() - (main_current[0].clone().into() + main_current[1].clone().into())) + (AB::Expr::ONE - main_current[3].clone().into()) * (main_next[0].clone().into() - main_current[0].clone().into() * main_current[1].clone().into()));
    }
}