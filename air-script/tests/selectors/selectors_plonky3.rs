...
impl<F: Field> AirScriptAir<F> for SelectorsAir {
    ...
    where AB: AirScriptBuilder<F = F>,
    {
        ...
        builder.when_transition().assert_zero_ext::<_>(main_current[0].clone().into() * main_current[1].clone().into() * main_current[2].clone().into() * (main_next[3].clone().into() - main_current[3].clone().into()) + (AB::Expr::ONE - main_current[1].clone().into()) * (AB::Expr::ONE - main_current[2].clone().into()) * (main_next[3].clone().into() - AB::Expr::ONE));
    }
}
