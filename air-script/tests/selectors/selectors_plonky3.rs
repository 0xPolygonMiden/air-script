...
impl<AB: AirBuilderWithPublicValues + AirBuilderWithPeriodicColumns> Air<AB> for SelectorsAir {
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero::<_>(main_current[0] * main_current[1] * main_current[2] * (main_next[3] - main_current[3]) + (AB::Expr::from(AB::F::from_u64(1)) - main_current[1]) * (AB::Expr::from(AB::F::from_u64(1)) - main_current[2]) * (main_next[3] - AB::Expr::from(AB::F::from_u64(1))));
    }
}