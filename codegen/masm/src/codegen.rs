...
use crate::exemption_points::gen_get_exemptions_points;
...
impl<'ast> CodeGenerator<'ast> {
    ...
    fn gen_compute_integrity_constraint_divisor(&mut self) -> Result<(), CodegenError> {
        ...
        self.writer.load_quadratic_element(
    ...
    fn gen_get_exemptions_points(&mut self) -> Result<(), CodegenError> {
        ...
        gen_get_exemptions_points(&mut self.writer)?;
        self.writer.end();
...
impl<'ast> AirVisitor<'ast> for CodeGenerator<'ast> {
    ...
    fn visit_boundary_constraint(
        ...
        // first _and_ the boundary constraints being iterated in natural order.
        self.writer.load_quadratic_element(
            self.config.composition_coef_address,
            self.composition_coefficient_count,
        )?;
        ...
        match op {
            ...
            Operation::Exp(left, exp) => {
                ...
                while e != 0 {
                    ...
                    self.writer.quadratic_element_square(e.trailing_zeros());
    ...
    fn visit_value(&mut self, value: &'ast Value) -> Result<Self::Value, Self::Error> {
        match value {
            ...
            Value::TraceElement(access) => {
                ...
                self.writer
                    .load_quadratic_element(base_address, target_element)?;
            }
            ...
            Value::PublicInput(name, index) => {
                ...
                let index: u32 = (start_offset + *index)
                    ...
                    .or(Err(CodegenError::InvalidIndex))?;
                self.writer
                    .load_quadratic_element(self.config.public_inputs_address, index)?;
            }
