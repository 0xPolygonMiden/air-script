...
use air_parser::{
    LexicalScope,
    ast::{self, Access, AccessType, ScalarExpr, TraceSegmentId},
    symbols,
};
use air_pass::Pass;
use air_types::*;
...
use crate::{
    ...
    ir::{
        ...
        MirAccessType, MirValue, Mul, Op, Owner, Parameter, PublicInputAccess,
        ...
        for trace_segment in &ast_eval.params {
            ...
            for binding in &trace_segment.bindings {
                ...
                let param_ty = binding
                    .ty
                    .access(AccessType::Index(Box::new(ScalarExpr::Const(Span::new(span, 0)))))
                    .ok()
                    .or(ty!(felt));
                for param in params {
    ...
    fn translate_call(&mut self, call: &'a ast::Call) -> Result<Link<Op>, CompileError> {
        ...
        if call.is_builtin() {
                ...
                self.mir.constraint_graph().get_evaluator_root(&resolved_callee)
            {
                ...
                // let arg_kinds = arg_nodes.iter().map(|arg|
                // arg.kind().unwrap()).collect::<Vec<_>>();
                let arg_kinds_refs = arg_kinds.iter().collect::<Vec<_>>();
                if !callee_ref.func_ty.check_args_kinds(&arg_kinds_refs) {
                    self.diagnostics
                        .diagnostic(Severity::Error)
            ...
            AccessType::Default => {
                ...
                Ok(Some(
                    Value::builder()
                        .value(SpannedMirValue {
                            span: access.span(),
                            value: MirValue::PublicInputTable(public_input_table.clone()),
                        })
                        .ty(public_input_table.ty())
                        .build(),
                ))
            },
            AccessType::Index(index) => {
                    ...
                    AccessType::Default => {
                        ...
                        Some(
                            Value::builder()
                                .value(SpannedMirValue {
                                    span: access.span(),
                                    value: MirValue::TraceAccessBinding(tab.clone()),
                                })
                                .ty(tab.ty())
                                .build(),
                        )
