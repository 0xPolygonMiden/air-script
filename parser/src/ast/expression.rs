        ...
        if self.module.name() == "$builtin" {
            match self.item {
                NamespacedIdentifier::Function(id) => {
                    matches!(id.name(), symbols::Sum | symbols::Prod | symbols::AsBool)
                },
                _ => false,
            }
        } else {
...
impl Typing for Expr {
    ...
    fn ty(&self) -> Option<Type> {
        match self {
            ...
            Self::Vector(vector) => vector.ty(),
            Self::Matrix(matrix) => matrix.ty(),
            Self::SymbolAccess(access) => access.ty,
            Self::Binary(bin_expr) => bin_expr.ty(),
            Self::Call(call) => call.ty,
            Self::ListComprehension(lc) => lc.ty,
            Self::Let(let_expr) => let_expr.ty(),
            Self::BusOperation(_) | Self::Null(_) | Self::Unconstrained(_) => ty!(felt),
        }
    }
    ...
    fn access_slice(
        ...
        match access_type {
            ...
            AccessType::Slice(range) => {
                ...
                match ty {
                    Type::Scalar(_) => unreachable!(),
                    Type::Vector(..) if slice_range.end > blen => {
                        Err(InvalidAccessError::IndexOutOfBounds)
                    },
                    Type::Vector(sty, _) => Ok(Self {
...
impl Call {
    pub fn new(span: SourceSpan, callee: Identifier, args: Vec<Expr>) -> Self {
        ...
        match callee.name() {
            ...
            symbols::AsBool => Self::as_bool(span, args),
            _ => Self {
                span,
    ...
    pub fn sum(span: SourceSpan, args: Vec<Expr>) -> Self {
        ...
        Self::new_builtin(span, "sum", args, ty!(felt).unwrap())
    }
    ...
    /// Constructs a function call for `as_bool`.
    /// An `as_bool(x)` is equivalent to an `enf x^2 = x plus a cast from felt to bool`.
    #[inline]
    pub fn as_bool(span: SourceSpan, args: Vec<Expr>) -> Self {
        //Self::new_builtin(span, "as_bool", args, ty!(felt).unwrap())
        let builtin_module = Identifier::new(SourceSpan::UNKNOWN, Symbol::intern("$builtin"));
        let name = Identifier::new(span, Symbol::intern("as_bool"));
